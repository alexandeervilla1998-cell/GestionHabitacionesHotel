...
<div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between bg-white p-4 border-2 border-gray-200 rounded animate-hotel-fade">
    <form action="{{ route('servicios.index') }}" method="GET" class="flex flex-col gap-3 w-full sm:flex-row sm:items-center">
        <!-- Search bar -->
        <div class="relative flex-grow">
            <span class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none text-gray-400">
                <i data-lucide="search" class="w-4 h-4"></i>
            </span>
            <input type="text" name="buscar" value="{{ request('buscar') }}" placeholder="Buscar por nombre o precio..." class="hotel-input pl-9" style="padding-left: 2.25rem;">
        </div>
        ...
        <div class="flex gap-2">
            ...
            @if(request('buscar') || request('estado') === 'inactivo')
                <a href="{{ route('servicios.index') }}" class="hotel-btn hotel-btn-secondary">
                    Limpiar
                </a>
            @endif
        </div>
                    ...
                    <tr x-show="expanded || {{ $loop->index }} < limit" x-transition>
                        ...
                        <td>
                            <div class="flex justify-end gap-2">
                                <a href="{{ route('servicios.edit', $servicio->id) }}" class="hotel-btn hotel-btn-secondary">
